...
syms x1 x2 real;
alpha = [2.20];              % Range of alpha values for AB(x) =< -alpha*B(x) + Beta
gx = [2.5];                  % Range of noise values
barrdeg =[1];
rho = 1;                    % Value over which we want to stay below
N = 3.0;                    % Time horizon (max time)
    ...
    for ii = 1:length(gx)
        for jj = 1:length(alpha)
            while count < maxcount
                if count == 1
                    u = 0;  % Control Input
                    [returnprob, retalpha, retbeta, retgam, Bx, prog] = cs3_verifctrl(alpha(jj), gx(ii), rho, N, x0, u, deg)
                    ...
                    [u, traceQ, cval, Q] = cs3_initux(1, Bx, alpha_st(jj, ii), beta(end), gx(ii),cfloor)
                    [returnprob, retalpha, retbeta, retgam, Bx, prog] = cs3_verifctrl(alpha(jj), gx(ii), rho, N, x0, u, deg)
...
title(['Probabilities vs. $\sigma$ (T = '+ string(N) + ' sec)'],'Interpreter','latex','Fontsize',16)
